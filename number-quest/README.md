...
The icons in `assets/` are placeholders (the turtle mascot on the blue/green gradient). To swap in your own artwork, replace those five files with new PNGs **at the exact same sizes and filenames**:
...
- **Icons:** edit the `ICONS` array at the top of `js/game.js` to change what kids count with.
- **Colors:** all colors are CSS variables at the top of `css/style.css` (`--blue-*`, `--green-*`).
- **Praise phrases:** edit the `PRAISE` array in `js/game.js`.
- **Mission map shape:** the `COLS` array and `ROW_HEIGHT` constant in `js/game.js` control the zig-zag layout of the 100 mission nodes.
